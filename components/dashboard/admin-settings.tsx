...
import { Settings, Globe, Gamepad2, Loader2, Save, X, Plus, ChevronDown, Tag, Camera } from 'lucide-react'
...
export function AdminSettings() {
  ...
  const handleUpdateTemplateImage = async (templateId: string, file: File) => {
    if (!file) return
    ...
    try {
      ...
      if (updateErr) {
        toast.error('Error al actualizar plantilla: ' + updateErr.message)
      } else {
        setTemplates(prev => prev.map(t => t.id === templateId ? { ...t, logo_url: newLogoUrl } : t))
        toast.success('Logo de la plantilla actualizado con éxito')
      }
    } catch (err: any) {
            ...
            <div className="lg:col-span-1 space-y-4 bg-background p-5 rounded-xl border border-border">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  {/* Input with embedded responsive Añadir button */}
                  <div className="relative mb-2">
                    <input
                      ...
                      className="w-full rounded-md border border-border bg-surface pl-3 pr-20 py-2 text-xs sm:text-sm text-white focus:border-primary focus:outline-none placeholder:text-muted-foreground/60"
                    ...
                    <button
                      ...
                      className="absolute right-1.5 top-1/2 -translate-y-1/2 px-2.5 py-1 rounded bg-primary/20 hover:bg-primary/30 text-primary text-xs font-600 disabled:opacity-30 transition-colors flex items-center gap-1"
                    >
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {templates.map(tmpl => (
                    <div key={tmpl.id} className="relative flex items-center gap-4 bg-background border border-border p-4 rounded-xl hover:border-primary/50 transition-colors">
                      ...
                      {/* Interactive Logo with hover/click to change */}
                      <div className="relative group/avatar w-14 h-14 rounded-lg overflow-hidden border border-border bg-surface shrink-0">
                        <img 
                        ...
                        />
                        <label 
                          htmlFor={`tmpl-img-${tmpl.id}`}
                          className="absolute inset-0 bg-black/70 opacity-0 group-hover/avatar:opacity-100 flex flex-col items-center justify-center cursor-pointer transition-opacity text-white"
                          title="Cambiar imagen de la plantilla"
                        >
                          {uploadingTemplateId === tmpl.id ? (
                            <Loader2 className="w-4 h-4 animate-spin text-primary" />
                          ) : (
                            <>
                              <Camera className="w-4 h-4 text-white" />
                              <span className="text-[8px] font-700 uppercase tracking-tight mt-0.5">Editar</span>
                            </>
                          )}
                          <input 
                            id={`tmpl-img-${tmpl.id}`}
                            ...
                            accept="image/*"
                            className="hidden"
                            disabled={uploadingTemplateId === tmpl.id}
                            onChange={(e) => {
                              const file = e.target.files?.[0]
                              if (file) handleUpdateTemplateImage(tmpl.id, file)
                            }}
                          />
                        </label>
                      </div>
